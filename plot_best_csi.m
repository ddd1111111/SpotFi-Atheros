clear;
close all;
load('C:\Users\user\Desktop\戈\20210605\output\zxc-1\7_p_num.mat')
load('C:\Users\user\Desktop\戈\20210605\clear0\zxc-1\7.mat');
load('C:\Users\user\Desktop\戈\20210605\clear0\zxc-1\7.mat');
load('C:\Users\user\Desktop\戈\20210605\output\zxc-1\7_best_removed.mat')
...
for calculate_number=1:180
    ...
    if p_num(calculate_number)==1
        figure(2);
        xlim([1 56]);ylim([-2*pi 2*pi]);hold on;title('程ㄎà');xlabel('更猧');ylabel('');
    ...
    figure(1);
    subplot(1,2,1);
    xlim([1 56]);ylim([-2*pi 2*pi]);hold on;title('﹍');xlabel('更猧');ylabel('');
    plot(1:56,phase(csi_0),'r')
    plot(1:56,phase(csi_1),'g')
    plot(1:56,phase(csi_2),'b')
    %
    ...
    subplot(1,2,2)
    xlim([1 56]);ylim([-2*pi 2*pi]);hold on;title('珼匡');xlabel('更猧');ylabel('');
...
legend('ぱ絬1材1更猧','ぱ絬3材1更猧')

figure(4)
plot(1:180,best_AoA);xlabel('');ylabel('à');ylim([-90 90]);
