function [Parameters, varargout] = backscatterEstimationMusic(csi_trace, M, N, c, fc, Ttot, fgap, SubCarrInd, d, paramRange, maxRapIters, useNoise, do_second_iter, MUSIC_iter, varargin)
% INPUTS:
...
for i = 1:MUSIC_iter
csi_trace_s = csi_trace(i,:);
%% formatting the input CSI matrix
if ~isfield(paramRange,'X')
    X.H{i} = formatCSI(csi_trace_s, N, M, Ttot, K, L, T);
else
    X = paramRange.X;
end
end
...
%% Applying MUSIC 
[Pn,Ps,Qn,Qs,EigenInfo] = GetQnBackscatter(X,EigDiffCutoff, nComps, MUSIC_iter);
